...
def test_asr_service(audio_file: str) -> bool:
...
def test_separation_service(audio_file: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get('success'):
                ...
                output_dir = Path("temp/cloud_test_output")
                ...
                if background_b64:
                    bg_path = output_dir / "background.mp3"
...
def main():
    ...
    # Test 2: ASR Service
    results['asr'] = test_asr_service(str(audio_file))

    # Test 3: Separation Service
    results['separation'] = test_separation_service(str(audio_file))
    ...
    if audio_file.exists():
        audio_file.unlink()
        rprint(f"\n[yellow]🧹 Cleaned up temporary audio file[/yellow]")
