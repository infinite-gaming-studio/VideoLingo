...
def check_hf_mirror():
    ...
    best_time = float('inf')
    vprint("[cyan]🔍 Checking HuggingFace mirrors...[/cyan]")
    for name, domain in mirrors.items():
        ...
        if result.returncode == 0:
            ...
            vprint(f"[green]✓ {name}:[/green] {response_time:.2f}s")
    if best_time == float('inf'):
        vprint("[yellow]⚠️ All mirrors failed, using default[/yellow]")
    vprint(f"[cyan]🚀 Selected mirror:[/cyan] {fastest_url} ({best_time:.2f}s)")
    return fastest_url
...
def transcribe_audio_local(raw_audio_file, vocal_audio_file, start, end):
    ...
    if torch.cuda.is_available():
        ...
        vprint(f"[green]🎮 Using CUDA GPU:[/green] {torch.cuda.get_device_name(0)}")
        vprint(f"[cyan]💾 GPU memory:[/cyan] {gpu_mem:.2f} GB, [cyan]📦 Batch size:[/cyan] {batch_size}, [cyan]⚙️ Compute type:[/cyan] {compute_type}")
    elif torch.backends.mps.is_available():
        device = "mps"
        batch_size = 4  # MPS建议使用较小的batch_size
        compute_type = "float16"
        vprint(f"[green]🍎 Using Apple Silicon MPS[/green]")
        vprint(f"[cyan]📦 Batch size:[/cyan] {batch_size}, [cyan]⚙️ Compute type:[/cyan] {compute_type}")
    else:
        device = "cpu"
        batch_size = 1
        compute_type = "int8"
        vprint(f"[yellow]💻 Using CPU (no GPU detected)[/yellow]")
        vprint(f"[cyan]📦 Batch size:[/cyan] {batch_size}, [cyan]⚙️ Compute type:[/cyan] {compute_type}")

    vprint(f"🚀 Starting WhisperX using device: {device} ...")
    vprint(f"[green]▶️ Starting WhisperX for segment {start:.2f}s to {end:.2f}s...[/green]")
    ...
    if os.path.exists(local_model):
        vprint(f"[green]📥 Loading local WHISPER model:[/green] {local_model} ...")
        model_name = local_model
    else:
        vprint(f"[green]📥 Using WHISPER model from HuggingFace:[/green] {model_name} ...")

    vad_options = {"vad_onset": 0.500,"vad_offset": 0.363}
    asr_options = {"temperatures": [0],"initial_prompt": "",}
    whisper_language = None if 'auto' in WHISPER_LANGUAGE else WHISPER_LANGUAGE
    vprint("[bold yellow] You can ignore warning of `Model was trained with torch 1.10.0+cu102, yours is 2.0.0+cu118...`[/bold yellow]")
    ...
    vprint("[bold green]Note: You will see Progress if working correctly ↓[/bold green]")
    result = model.transcribe(raw_audio_segment, batch_size=batch_size, print_progress=True)
    transcribe_time = time.time() - transcribe_start_time
    vprint(f"[cyan]⏱️ time transcribe:[/cyan] {transcribe_time:.2f}s")
    ...
    vprint(f"[cyan]⏱️ time align:[/cyan] {align_time:.2f}s")
